...
class TreeItem extends RoleType {
	static {
		sprockets.withAria(this, {
		role: 'treeitem',
		selected: {
...
class ScrollBox extends RoleType {
	static {
		sprockets.withAria(this, {
		role: 'frame',
		activedescendant: {
...
class SwitchBox extends RoleType {
	static {
		sprockets.withAria(this, {
		role: 'group',
		owns: {
...
class Table extends RoleType { // FIXME uses className. This shouldn't be hard-wired
	getTable() {
		let element = this.element;
	...
	sort(column, type, reverse) {
		...
		for (let j=0, m=tBodies.length; j<m; j++) {
			...
			switch (type) {
				...
				default: throw Error('Unrecognized sort type: ' + type);
			}
