...
describe('CustomNamespace', () => {
  ...
  test('creates xml-style namespace with colon prefix', () => {
    const ns = new CustomNamespace({ urn: 'TestURN', name: 'test', style: 'xml' });
    expect(ns.prefix).toBe('test:');
    expect(ns.urn).toBe('TestURN');
  });

  test('creates vendor-style namespace with hyphen prefix', () => {
    const ns = new CustomNamespace({ urn: 'TestURN', name: 'test', style: 'vendor' });
    expect(ns.prefix).toBe('test-');
  });

  test('lowercases name and style', () => {
    const ns = new CustomNamespace({ urn: 'X', name: 'FOO', style: 'XML' });
    expect(ns.name).toBe('foo');
    expect(ns.prefix).toBe('foo:');
  });

  test('throws on invalid style', () => {
    expect(() => new CustomNamespace({ urn: 'X', name: 'foo', style: 'bad' })).toThrow();
  });

  test('throws on empty name', () => {
    expect(() => new CustomNamespace({ urn: 'X', name: '', style: 'xml' })).toThrow();
  });

  test('lookupTagName returns prefixed name', () => {
    const ns = new CustomNamespace({ urn: 'X', name: 'haz', style: 'xml' });
